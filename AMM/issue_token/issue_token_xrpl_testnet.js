...
if (args.length < 1) {
  console.error("Usage: node script.js <TOKEN_NAME>")
  process.exit(1)
}
const currency_code = args[0]

// Validate the currency code
if (currency_code.length !== 3) {
  console.error("Error: The token name must be exactly 3 characters long.")
  process.exit(1)
}
...
async function main() {
  ...
  console.log(`Got hot address ${hot_wallet.address} with secret ${hot_wallet.seed}`)
  console.log(`Got cold address ${cold_wallet.address} with secret ${cold_wallet.seed}`)
  console.log(`Got customer_one address ${customer_one_wallet.address} with secret ${customer_one_wallet.seed}`)
  console.log(`Got customer_two address ${customer_two_wallet.address} with secret ${customer_two_wallet.seed}`)
